...
export default function ReplayPage() {
  ...
  return (
    <div className="p-6 grid grid-cols-3 gap-6">
      <div>
        <div className="text-xs text-ink2 uppercase tracking-wide mb-2">Recorded runs</div>
        <div className="border border-line bg-panel rounded-lg divide-y divide-line max-h-[520px] overflow-y-auto">
          ...
          {runs.map((r) => (
            ...
            >
              <div className="font-mono text-ink">{r.run_id}</div>
              <div className="text-xs text-ink3 mt-0.5">{r.scenario} · {r.event_count} events</div>
            </button>
          ))}
        </div>
      </div>
          ...
          <>
            <div className="flex items-center gap-3 text-xs font-mono text-ink2">
              <button
                ...
                className="px-3 py-1.5 rounded border border-line2 text-ink hover:border-teal"
              >
                {playing ? 'pause' : 'play'}
              </button>
              <select
                ...
                className="bg-panel border border-line2 rounded px-2 py-1.5 text-ink"
              >
